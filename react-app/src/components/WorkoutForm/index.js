...
const WorkoutForm = ({ isModalOpen, setIsModalOpen }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        const res = fields.filter(field => !field.repetitions || !field.sets)
        const exerciseIdArr = fields.map(field => field.exercise_id)
        if (!newWorkoutName) return setErrors(['Please name your workout plan.'])
